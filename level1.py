...
def level():
    ...
    npcs = [doomimp.DoomImp(x=230, y=650, target=player), doomimp.DoomImp(x=630, y=550, target=player), doomimp.DoomImp(x=580,y=960,target=player)]
#    npcs = []
    for npc in npcs:
    ...
    bricks = []
    for i in range(1,8):
        bricks.append(brick.Brick(size=40, x=300 + 125*i, y=200))
    for i in range(1,20):
        bricks.append(brick.Brick(size=10, y=550, x=130+21*i))
    for i in range(1,20):
        bricks.append(brick.Brick(size=10, y=850, x=130+21*i))
    for i in range(1,20):
        bricks.append(brick.Brick(size=10, y=750, x=130+21*i))
    ...
    staticbricks = []
    staticbricks.append(brick.StaticBrick(x=455, y=450, width=50, height=10))
    staticbricks.append(brick.StaticBrick(x=380, y=670, width=10, height=420))
    staticbricks.append(brick.StaticBrick(x=400, y=1340, width=800, height=10))

    for staticbrick in staticbricks:
        framework.space.add(staticbrick.shape)

    boundary = []    
    boundary.append(brick.StaticBrick(x=1500, y=6550, width=3000, height=10))
    boundary.append(brick.StaticBrick(x=1500, y=10, width=3000, height=100,invisible=True))
    for bound in boundary:
        framework.space.add(bound.shape)

    maze.make_maze(130, 480, width=8, height=4, cellwidth=70, cellheight=65, wallthickness=10)    

    maze.make_maze(570, 480, width=8, height=4, cellwidth=70, cellheight=65, wallthickness=10)

    maze.make_maze(870, 480, width=80, height=12, cellwidth=100, cellheight=75, wallthickness=20)
    
    maze.make_maze(2500, 480, width=30, height=12, cellwidth=100, cellheight=75, wallthickness=20)
    
    
    framework.grabbables.append((brick.Balloon(radius=15, num_pts=6, x=330, y=900)))
    ...
    framework.grabbables.append((brick.Balloon(radius=15, num_pts=10, x=145, y=925)))
    ...
    candies = []
    for i in range(10):
        candies.append(sprite.Basic_Sprite("images/candy.png",530,700,width=10,height=8,mass=.1))
    ...
    barrel = sprite.Basic_Sprite("images/barrel.png",460,400,width=15,height=33,mass=10,jumpable=1,elasticity=1,offset=(0,-1))
    framework.space.add(barrel.shape)
    framework.space.add(barrel.body)
    barrel.add(framework.characters)

    global airship1
    airship1 = airship.Airship(x=500,y=350)
